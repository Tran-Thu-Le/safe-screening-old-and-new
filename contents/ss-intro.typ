...
Another landmark appearred when Ogawa #etal @ogawaSafeScreeningNonSupport introduced the _Sample Elimination_ for sparse (soft) Support Vector Machine (SVM) for discarding the non-support vectors in SVM models. In the language of optimization, this is equivalent to a dimensionality reduction for the dual problem of SVM.

Furthermore, Shibagaki #etal @shibagakiSimultaneousSafeScreening2016 propose the safe screening techniques for simultaneous sample and feature elimination for sparse SVM. 


#remLE[Safe screening also has three paradisms:
- Static screening 
